# Reference: https://github.com/sheffieldnlp/naacl2018-fever/blob/master/src/scripts/rte/da/train_da.py
...
from rte.da.util_random import SimpleRandom
...
def train_model(params: Union[Params, Dict[str, Any]], cuda_device:int, serialization_dir: str) -> Model:
    """
    ...
    sys.stdout = TeeLogger(os.path.join(serialization_dir, "stdout.log"), sys.stdout, file_friendly_terminal_output=False)  # type: ignore
    sys.stderr = TeeLogger(os.path.join(serialization_dir, "stderr.log"), sys.stderr, file_friendly_terminal_output=False)  # type: ignore
    ...
    dataset_reader = FEVERReader(wiki_tokenizer=Tokenizer.from_params(ds_params.pop('wiki_tokenizer', {})),
...
if __name__ == "__main__":
    ...
    train_model(params,args.cuda_device,args.logdir)