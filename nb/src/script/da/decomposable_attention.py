# Reference: https://docs.allennlp.org/v0.9.0/api/allennlp.models.decomposable_attention.html
# GitHub code could not be located
...
class DecomposableAttention(Model):
    ...
    def __init__(self, vocab: Vocabulary,
        ...
        # COMMENT: Add support for F1 Score metric
        self._f1_macro = FBetaMeasure(beta=1.0, average="macro")
        self._f1_micro = FBetaMeasure(beta=1.0, average="micro")
    ...
    def forward(self,  # type: ignore
        ...
        if label is not None:
            ...
            # COMMENT: Add support for F1 measure
            self._f1_macro(label_logits, label)
            self._f1_micro(label_logits, label)
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        f1_macro = {f"macro_{k}": v for k, v in self._f1_macro.get_metric(reset).items()}
        f1_micro = {f"micro_{k}": v for k, v in self._f1_micro.get_metric(reset).items()}

        all_metrics = {'accuracy': self._accuracy.get_metric(reset)}
        all_metrics.update(f1_macro)
        all_metrics.update(f1_micro)
