...
def plot_finetune(model_path: Path, metric: str = "accuracy", return_metric: bool = False):
    metrics = []
    ...
    for c in metrics.filter(like=metric).columns:
        if "best" not in c:            
            ax = metrics.plot(x="epoch", y=c, ax=ax)
